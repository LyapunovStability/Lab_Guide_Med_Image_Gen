...
try:
    ...
    # Fallback to standard LayerNorm if apex is not available
    FusedLayerNorm = nn.LayerNorm
    print("Warning: apex.normalization.FusedLayerNorm not available. Using standard LayerNorm.")
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if swiglu:
            if xops is None:
                raise ImportError("xformers is required for swiglu=True. Please install xformers.")
...
@register_model
def eva02_base_patch16_xattn_fusedLN_NaiveSwiGLU_subln_RoPE(pretrained=False, **kwargs):
    model = VisionTransformer(
        patch_size=16, 
        embed_dim=768, 
        depth=12, 
...
@register_model
def eva02_small_patch16_xattn_fusedLN_SwiGLU_preln_RoPE(pretrained=False, **kwargs):
    model = VisionTransformer(
        patch_size=16, 
        embed_dim=384, 
        depth=12, 
...
@register_model
def eva02_tiny_patch16_xattn_fusedLN_SwiGLU_preln_RoPE(pretrained=False, **kwargs):
    model = VisionTransformer(
        patch_size=16, 
        embed_dim=192, 
        depth=12, 
